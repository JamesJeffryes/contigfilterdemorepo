package bsadkhinContigFilterApplicationDemo::bsadkhinContigFilterApplicationDemoClient;
...
bsadkhinContigFilterApplicationDemo::bsadkhinContigFilterApplicationDemoClient
...
sub new
{
    ...
    my $self = {
	client => bsadkhinContigFilterApplicationDemo::bsadkhinContigFilterApplicationDemoClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a bsadkhinContigFilterApplicationDemo.FilterContigsParams
$output is a bsadkhinContigFilterApplicationDemo.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a bsadkhinContigFilterApplicationDemo.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a bsadkhinContigFilterApplicationDemo.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "bsadkhinContigFilterApplicationDemo.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "bsadkhinContigFilterApplicationDemo.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "bsadkhinContigFilterApplicationDemo.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for bsadkhinContigFilterApplicationDemo::bsadkhinContigFilterApplicationDemoClient\n";
    }
    if ($sMajor == 0) {
        warn "bsadkhinContigFilterApplicationDemo::bsadkhinContigFilterApplicationDemoClient version is $svr_version. API subject to change.\n";
    }
}
...
package bsadkhinContigFilterApplicationDemo::bsadkhinContigFilterApplicationDemoClient::RpcClient;
